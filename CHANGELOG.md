...
- **A2A push notification configuration**: v1 JSON-RPC and gRPC clients can create, get, list, and delete persistent per-task callback configurations. Callback URLs receive SSRF validation and credentials are encrypted in PostgreSQL when secret encryption is configured.
