...
- **OpenAI-compatible `/v1/chat/completions`**: `POST /v1/chat/completions` maps `model` to an AgentSystem, creates a Task from chat `messages`, waits for a terminal phase, and returns an OpenAI-shaped completion. Streaming requests open SSE immediately, send keepalives, and finish with the Task result. Sequential runs now persist the final agent result in `status.output.last_output`. See [Call AgentSystems via OpenAI Chat Completions](docs/pages/guides/openai-chat-completions.md).
