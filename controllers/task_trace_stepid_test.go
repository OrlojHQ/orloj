...
func TestAppendTaskTraceDeterministicStepIDsByAttempt(t *testing.T) {
	...
	controller.appendTaskTrace(&task, crds.TaskTraceEvent{Type: "agent_start"})
	...
	controller.appendTaskTrace(&task, crds.TaskTraceEvent{Type: "agent_end"})
...
func TestAppendTaskTraceUsesExplicitStepIDAndStepFallback(t *testing.T) {
	...
	controller.appendTaskTrace(&task, crds.TaskTraceEvent{
		StepID:  "custom-step",
		Attempt: 4,
		Type:    "agent_event",
	})
	...
	controller.appendTaskTrace(&task, crds.TaskTraceEvent{Type: "agent_event"})
