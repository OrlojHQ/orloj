...
- `allowPrivate` (boolean): for trusted `http` and `external` tools only, permits private/internal endpoint addresses. Link-local, cloud metadata, and unspecified addresses remain blocked.
...
- `allowPrivate` defaults to `false`; set it only for trusted in-cluster/private `http` or `external` tool gateway endpoints.
